...
total.newU <- total.new[!duplicated(total.new$eid), c("eid", "sex", "age", "death", "death_date", "endfollowup", "startfollowup","PRS_0.5","I25","birth_date","ac_date")]
...
for (i in 1:length(icd.all)) {
    ...
    res <- rbind(res, c(mod,mod1,mod2,mod4))
...
colnames(resdf) <- c("ICD10","beta_main","z_main","beta_interaction","z_interaction","beta_just_mi","z_just_mi","beta_prs_icd","z_prs_icd")
write.table(resdf, file = paste0(path,'survivalanalysis_post.tsv'), sep = "\t", row.names = FALSE, col.names = TRUE, quote=F)
...
resdf <- read.table(file = paste0(path,'survivalanalysis_post.tsv'), sep = "\t", header = TRUE, stringsAsFactors = F)
...
## PLOT INTERACTION BETWEEN PRS and ICD for association with MI ## (Survival  bias)
resdf <- read.table(file = paste0(path,'survivalanalysis_post.tsv'), sep = "\t", header = TRUE, stringsAsFactors = F)
icd.sel <- resdf$ICD10[2*pnorm(-abs(resdf$z_main)) < (0.05/nrow(resdf))]
resdfS <- resdf[resdf$ICD10 %in% icd.sel, ]
resdfS$p_interaction_mi <- 2*pnorm(-abs(resdfS$z_interaction))
resdfS$logp <- -log10(resdfS$p_interaction_mi)
resdfS$ICD10label <- resdfS$ICD10
resdfS$ICD10label[resdfS$logp < -log10(0.05 / length(icd.all))] <- ""
ggplot(resdfS, aes(ICD10,logp)) + 
  geom_point() + 
